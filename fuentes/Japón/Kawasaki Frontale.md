# Kawasaki Frontale (川崎フロンターレ)

- **Hit vía documento consolidado de la liga (2026-09-13).** Aparece en la tabla de clubes de J1
  bajo la abreviatura "川崎Ｆ"/"川崎F" (ancho variable entre años, ver gotcha en
  `fuentes/Japón/_notas-generales.md`) — confirmado presente en 2012, 2015, 2018, 2020, 2023 y 2025.
  En la edición 2025, Kawasaki Frontale es el segundo club de mayor ingreso de la liga (100 億円/
  ¥10.069M), junto con Urawa el único par que superó los ¥10.000M ese ejercicio.
  Serie de ejemplo (edición 2025, millones de JPY): 川崎Ｆ 売上高 2023: 7.963, 2024: 8.403, 2025:
  10.069 (+1.666).
  Los 14 PDFs de la liga (2012-2025) están en
  `Clubes/Japón/_J.League (todos los clubes)/club_doc-2012.pdf` a `club_doc-2025.pdf` — ver
  `fuentes/Japón/_notas-generales.md` para la metodología completa y los gotchas (costos solo a
  nivel de división, no por club).
- Pendiente: transcribir a Markdown y extraer la serie completa de los 14 PDFs. No se buscó balance
  individual de la sociedad operadora (株式会社川崎フロンターレ, del grupo Fujitsu) — el documento
  de la liga ya cubre lo pedido.
- Contacto: `aboutj.jleague.jp/corporate/assets/pdf/club_info/club_doc-<año>.pdf`.
- Último chequeo: 2026-09-13.
- **CARGADO al sitio (2026-09-13), Ejercicio 2025**: ver `data/kawasakifrontale-data.js`. Solo
  Sponsor (¥3.765M)/Gate (¥1.469M)/Total (¥10.069M) son reales por club — el resto de categorías y
  TODOS los costos solo se publican a nivel de división, no por club (ver corrección en
  `_notas-generales.md`). `expenseLinesByYear` vacío a propósito, `officialTotalExpenses`/
  `officialPAT` en `null`.
