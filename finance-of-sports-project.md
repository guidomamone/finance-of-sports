...
## Versión 127: lo que una nota interna no tiene que contarle al visitante

Guido revisó la Versión 126 y trajo seis observaciones. Cinco eran de forma (sacar el card "Sobre mí", la página que no aprovechaba el ancho, la lista de cards que convenía que fuera tabla, los em dashes que violan una regla de estilo escrita en la Versión 42, y la sospecha de que los totales de la página estaban hardcodeados, que no lo estaban). La sexta era de fondo: **"veo que en Salvedades a veces pones 'descargado por Guido' o direcciones que son de mi computadora. Eso no puede aparecerle al usuario."**

Tenía razón, y medirlo lo puso peor de lo que sonaba: **las 91 entradas de `sources{}` tienen `note`, y 64 mencionan su nombre o rutas del repo.** Promedian 640 caracteres. Lo que el sitio le estaba mostrando a un visitante incluía "PDF subido directamente por Guido", "Transcripción completa de la cuenta de resultados en Clubes/España/Athletic Club/..." y el detalle de con qué herramienta se hizo cada OCR.

El error no fue de redacción de esas notas: fue de criterio al publicarlas. `note` nunca fue un campo público. Es lo que una sesión le deja a la siguiente para no tener que redescubrir dónde quedó una transcripción o por qué un número se leyó de una página y no de otra. La Versión 126 la renderizó tal cual porque era el único texto disponible, sin preguntarse para quién estaba escrito.

El arreglo separa los dos usos, y la parte interesante es cuánto NO hizo falta escribir a mano. De las 29 entradas que tenían algo para aclarar, 24 se podían deducir de campos que ya existían: que el tipo de cambio sea una referencia y no una cotización del documento, que el balance no publique el resultado del ejercicio, que el ejercicio no informe deuda ni caja. Eso lo arma ahora `sourceCaveats()` y se actualiza solo cuando cambian los datos. Quedó prosa nueva únicamente en 17 documentos, y casi toda es la misma frase repetida: los 10 clubes de Japón comparten la salvedad de que la J.League publica tres líneas por club y el resto por división.

La tercera pieza fue otra duplicación, esta en la capa de presentación: las etiquetas de tipo y nivel de documento ("Balance auditado", "Fuente primaria") estaban escritas dos veces en `js/finanzas-render.js` y dos veces más en `tools/generate-fuentes-page.js`, o sea que el sitio y la página podían describir distinto el mismo documento. Se mudaron a `data/sources-view.js`, que cargan los dos mundos: el navegador por `<script src>` y los generadores por `vm`.

Para que nada de esto vuelva solo, `tools/audit.js` sumó dos guardarraíles en P1: uno que falla si un `publicNote` menciona un nombre propio, una ruta del repo o un detalle de transcripción, y otro que falla si alguien vuelve a interpolar `.note` dentro de HTML. El primero se probó ensuciando a propósito la nota de River y verificando que saltara.

Los em dashes merecen una línea aparte, porque el patrón es el mismo que el del "88 claves sin traducir" de la versión anterior: la regla existía, escrita y con el motivo ("los odio a los em dashes", Versión 42), y aun así escribí 38 líneas nuevas con em dashes a lo largo de toda la sesión. Una regla documentada no se cumple sola si nadie la chequea. Quedaron limpias las 38 líneas propias y 15 títulos de documento, respetando la única excepción que la regla ya preveía: el "—" como símbolo de "sin dato" en una celda de tabla.
